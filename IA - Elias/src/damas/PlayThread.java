...
public class PlayThread extends Thread {
    ...
    private final boolean IS_KING;
    private final boolean IS_BACKWARD;
    private final String NAME;
    private int[][] M;

    public PlayThread(ArrayList<ArrayList<int[]>> jogadas, int[][] m, int max, int corAdversario, int i, int j, int d, boolean isKing, boolean isBackward, int signal_line, int signal_column, String name) {
    ...
    public void run() {
        ...
        for (int k = 1; k < MAX; k++) {
            ...
            if (compara(I + SIGNAL_LINE * k * D, J + SIGNAL_COLUMN * k, tab, COR_ADVERSARIO) || compara(I + SIGNAL_LINE * k * D, J + SIGNAL_COLUMN * k, tab, COR_ADVERSARIO + 2)) {
                if (compara(I + SIGNAL_LINE * 2 * k * D, J + SIGNAL_COLUMN * 2 * k, M, 0)) {
                    tab[I + k * D][J + k] = 0;

                    int[] posicao = {I + 2 * k * D, J + SIGNAL_COLUMN * 2 * k};

                    jogada.add(posicao);

                    adicionaNoFinal(jogadasTemp, concatenaListaJogadas(jogada, eatPiece(I + SIGNAL_LINE * 2 * k * D, J + SIGNAL_COLUMN * 2 * k, MAX, tab, D, jogada, false, NAME)));
            ...
            } else if (compara(I + SIGNAL_LINE * k * D, J + SIGNAL_COLUMN * k, tab, 0) && ((IS_KING && IS_BACKWARD) || (!IS_BACKWARD))) {
                ...
                if (comeu) {
                    adicionaNoFinal(jogadasTemp, concatenaListaJogadas(jogada, eatPiece(I + SIGNAL_LINE * k * D, J + SIGNAL_COLUMN * k, MAX, tab, D, jogada, false, NAME)));
                }
                //System.out.println("");
                //imprime(tab);
            } else {
