...
import static damas.Main.*;

/**
 *
 * @author Elias
 */
public class MIN_MAX_THREAD {
    ...
    public MIN_MAX_THREAD(int[][] tab, boolean type, int maxLevel) {
        this.TAB = tab;
        this.TYPE = type;
            ...
            for (int line = 0; line < state.getTAB().length; line++) {
                for (int column = 0; column < state.getTAB()[0].length; column++) {
                    ...
                    if ((((state.getTAB()[line][column] == 1) && TYPE) || ((state.getTAB()[line][column] == 2) && !TYPE))
                        ...
                        ArrayList<ArrayList<int[]>> lista = verificaJogada(line, column, state.getTAB());
                        ...
                        int maxCount = maxCount(lista, pos);                        
                        
                        for (int i = 0; i < lista.size(); i++) {

                            //Se a jogada for a máxima
                            if (count(lista.get(i), pos) == maxCount) {

                                //Atualiza o tabuleiro
                                int[][] child = updateTab(lista.get(i), state.getTAB(), pos, state.getTAB()[line][column]);
